...
<head>
    ...
    <style>
        ...
        .sub-nav-button.active, .inspector-input-tab.active { border-bottom: 2px solid var(--tg-button); color: var(--tg-button); font-weight: 600; }
...
<body class="flex flex-col h-screen">
    ...
    <main class="flex-grow overflow-hidden flex flex-col">
        ...
        <div id="chat-page" class="page active flex-col h-full">
            ...
            <div class="flex-shrink-0 flex items-center space-x-2 p-2 bg-[var(--tg-bg)] border-t border-[var(--tg-secondary-bg)]">
                ...
                <button id="generate-button" class="p-3 rounded-full transition-opacity btn-transition flex-shrink-0 bg-[var(--tg-button)] text-[var(--tg-button-text)]" aria-label="Сгенерировать">
                    <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" stroke="none"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>
                </button>
            </div>
        </div>
        ...
        <!-- СТРАНИЦА 3: ИНСПЕКТОР КОДА -->
        <div id="inspector-page" class="page flex-col h-full">
            <div class="flex-shrink-0 p-3 border-b border-[var(--tg-secondary-bg)] bg-[var(--tg-bg)]">
                <h2 class="text-xl font-bold text-center">Инспектор Кода</h2>
            </div>
            <!-- Вкладки для выбора метода ввода -->
            <div class="flex-shrink-0 flex border-b border-[var(--tg-secondary-bg)] bg-[var(--tg-bg)]">
                <button data-input-view="url" class="inspector-input-tab active w-1/2 p-3 text-center text-sm font-medium">По URL</button>
                <button data-input-view="direct" class="inspector-input-tab w-1/2 p-3 text-center text-sm font-medium text-[var(--tg-hint)]">Вставить код</button>
            </div>
            
            <!-- Контейнер для различных методов ввода -->
            <div id="inspector-inputs" class="flex-shrink-0">
                <!-- Ввод URL -->
                <div id="url-input-view" class="flex items-center space-x-2 p-2 bg-[var(--tg-bg)]">
                    <input id="url-input" type="url" placeholder="https://example.com" class="w-full p-3 rounded-full focus:outline-none bg-[var(--tg-secondary-bg)] text-[var(--tg-text)] placeholder-[var(--tg-hint)]">
                    <button id="load-url-button" class="p-3 rounded-full transition-opacity btn-transition flex-shrink-0 bg-[var(--tg-button)] text-[var(--tg-button-text)]" aria-label="Загрузить">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.47 5.47 0 00-1.1 1.143 4.002 4.002 0 00-7.8 1.857A4.002 4.002 0 003 15z" /></svg>
                    </button>
                </div>
                <!-- Прямой ввод кода -->
                <div id="direct-input-view" class="hidden p-2 flex flex-col">
                    <textarea id="direct-code-input" class="w-full h-24 p-2 rounded-lg bg-[var(--tg-secondary-bg)] text-[var(--tg-text)] placeholder-[var(--tg-hint)] font-mono text-sm" placeholder="Вставьте ваш HTML код сюда..."></textarea>
                    <button id="render-direct-code-button" class="mt-2 bg-[var(--tg-button)] text-[var(--tg-button-text)] py-2 px-4 rounded-lg btn-transition">Отобразить</button>
                </div>
            </div>

            <div class="flex-shrink-0 flex border-b border-[var(--tg-secondary-bg)] bg-[var(--tg-bg)]">
                <button data-view="inspector-code" class="sub-nav-button active w-1/2 p-4 text-center text-[var(--tg-hint)]">Код</button>
                <button data-view="inspector-preview" class="sub-nav-button w-1/2 p-4 text-center text-[var(--tg-hint)]">Превью</button>
            </div>
            <div id="inspector-code-view" class="flex-grow flex flex-col overflow-hidden">
                 <div class="flex-grow overflow-auto bg-[#2d2d2d]">
                    <pre><code id="inspector-code-output" class="language-html text-sm p-4 block whitespace-pre-wrap font-mono">// Вставьте URL или код и нажмите "Загрузить/Отобразить".</code></pre>
                </div>
            </div>
            <div id="inspector-preview-view" class="flex-grow hidden bg-white relative">
                <iframe id="inspector-preview-iframe" class="w-full h-full border-0"></iframe>
                <div id="inspector-loader" class="absolute inset-0 bg-white bg-opacity-75 flex-col items-center justify-center hidden">
                    <div class="loader" style="width: 32px; height: 32px;"></div>
                    <p class="mt-4 text-lg font-semibold text-[var(--tg-text)]">Загрузка...</p>
                </div>
            </div>
        </div>
        ...
        <div id="profile-page" class="page p-4 overflow-y-auto flex-col h-full space-y-4">
            ...
            <div id="referral-section" class="bg-[var(--tg-secondary-bg)] p-4 rounded-xl space-y-3">
                <h3 class="text-lg font-bold">Пригласите друзей</h3>
                <p class="text-sm text-[var(--tg-hint)]">Пригласите друга и получите бонус! Вы получите **10 генераций** за каждого друга, который оформит подписку. Ваш друг получит **1 бонусную генерацию** сразу после перехода по ссылке.</p>
                <div class="flex items-center space-x-2">
                    ...
                    <button id="copy-referral-link-button" class="p-2 rounded-lg bg-[var(--tg-button)] text-[var(--tg-button-text)] btn-transition flex-shrink-0">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                    </button>
                </div>
            </div>
        </div>
    ...
    <footer class="flex-shrink-0 h-20 bg-[var(--tg-bg)] border-t border-[var(--tg-secondary-bg)] flex justify-around items-center">
        <button data-page="chat-page" class="nav-button active flex flex-col items-center justify-center p-2 h-full" aria-label="Чат">
        ...
        <button data-page="creation-page" class="nav-button flex flex-col items-center justify-center p-2 h-full" aria-label="Создание">
        ...
        <button data-page="inspector-page" class="nav-button flex flex-col items-center justify-center p-2 h-full" aria-label="Инспектор">
            <svg class="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <span class="text-xs">Инспектор</span>
        </button>
        <button data-page="profile-page" class="nav-button flex flex-col items-center justify-center p-2 h-full" aria-label="Профиль">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const App = {
            config: {
                apiKey: 'YOUR_API_KEY', 
                // ЗАМЕНИТЕ 'YOUR_BOT_USERNAME' и 'YOUR_APP_NAME' на ваши значения
                botUsername: 'YOUR_BOT_USERNAME',
                appName: 'YOUR_APP_NAME',
                maxFreeGenerations: 5,
                storageKey: 'aiWebAppBuilderState_v6' // Изменена версия хранилища
            },
            state: {
                activeChatId: null,
                chats: [],
                dailyGenerations: { count: 0, lastReset: 0 },
                bonusGenerations: 0, // Новое состояние для бонусных генераций
                subscription: { isActive: false, endDate: null },
                isGenerating: false,
                referrerId: null, // ID пользователя, который пригласил текущего
            },
            elements: {},
            ...
            init() {
                ...
                this.referralManager.initialize(); // Инициализация реферальной системы
                this.specialAccessManager.grantPremiumIfNeeded();
                this.ui.renderAll();
            ...
            cacheDOMElements() {
                const ids = [
                    ...
                    'inspector-page', 'url-input', 'load-url-button', 
                    'inspector-code-view', 'inspector-preview-view', 
                    'inspector-code-output', 'inspector-preview-iframe', 'inspector-loader',
                    'url-input-view', 'direct-input-view', 'direct-code-input', 'render-direct-code-button',
                    // Новые элементы для профиля и реферальной программы
                    'user-profile-card', 'user-avatar', 'user-fullname', 'user-username',
                    'referral-section', 'referral-link-input', 'copy-referral-link-button'
                ];
                ids.forEach(id => {
                ...
                this.elements.subNavButtonsInspector = document.querySelectorAll('#inspector-page .sub-nav-button');
                this.elements.inspectorInputTabs = document.querySelectorAll('#inspector-page .inspector-input-tab');
            },
            ...
            stateManager: {
                load() {
                    ...
                    if (savedState) {
                        ...
                        // Объединяем сохраненное состояние с состоянием по умолчанию, чтобы добавить новые поля
                        Object.assign(App.state, parsedState);
                    } else {
                ...
                setDefaultState() {
                    ...
                    App.state.bonusGenerations = 0;
                    App.state.referrerId = null;
                },
                checkDateBasedState() {
            ...
            ui: {
                ...
                renderSubscriptionStatus() {
                    const { subscription, dailyGenerations, bonusGenerations } = App.state;
                    ...
                    if (subscription.isActive) {
                        ...
                        const remaining = maxFreeGenerations - dailyGenerations.count;
                        el.innerHTML = `
                            <p class="text-sm">Бесплатных генераций сегодня:</p>
                            <p class="text-2xl font-bold">${remaining} / ${maxFreeGenerations}</p>
                            ${bonusGenerations > 0 ? `<p class="text-sm text-blue-500 mt-1">Бонусных генераций: ${bonusGenerations}</p>` : ''}
                            <button id="subscribe-button" class="w-full mt-4 bg-yellow-400 text-black font-bold py-2 px-4 rounded-lg btn-transition">Подписаться за 999 Stars</button>`;
                ...
                toggleInspectorInputView(view) {
                    App.elements.inspectorInputTabs.forEach(b => {
                        b.classList.toggle('active', b.dataset.inputView === view);
                        b.classList.toggle('text-[var(--tg-hint)]', b.dataset.inputView !== view);
                    });
                    App.elements.urlInputView.classList.toggle('hidden', view !== 'url');
                    App.elements.directInputView.classList.toggle('hidden', view !== 'direct');
                },
                toggleInspectorView(view) {
                    App.elements.subNavButtonsInspector.forEach(b => b.classList.toggle('active', b.dataset.view === view));
                    App.elements.inspectorCodeView.classList.toggle('hidden', view !== 'inspector-code');
                    App.elements.inspectorPreviewView.classList.toggle('hidden', view !== 'inspector-preview');
                },
            ...
            eventManager: {
                addEventListeners() {
                    ...
                    App.elements.inspectorInputTabs.forEach(button => button.addEventListener('click', () => App.ui.toggleInspectorInputView(button.dataset.inputView)));
                    App.elements.loadUrlButton.addEventListener('click', () => App.inspectorManager.handleLoadUrl());
                    App.elements.urlInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') App.inspectorManager.handleLoadUrl(); });
                    App.elements.renderDirectCodeButton.addEventListener('click', () => App.inspectorManager.handleRenderDirectCode());
                    App.elements.subNavButtonsInspector.forEach(button => button.addEventListener('click', () => App.ui.toggleInspectorView(button.dataset.view)));
            ...
            specialAccessManager: {
                grantPremiumIfNeeded() {
                    if (App.tg.initDataUnsafe?.user && !App.state.subscription.isActive) {
                        const username = App.tg.initDataUnsafe.user.username;
                        if (username && username.toLowerCase() === 'yung_rezac') {
                            console.log('Granting special premium access to @yung_rezac');
                            App.paymentManager.activateSubscription(3650); 
                            App.tg.showPopup({
                                title: 'Особый доступ',
                                message: 'Вам предоставлен неограниченный премиум-доступ. Спасибо за вашу поддержку!',
                                buttons: [{ type: 'ok' }]
                            });
                        }
                    }
            ...
            referralManager: {
                initialize() {
                    // tgWebAppStartParam содержит параметр из реферальной ссылки
                    const referrerId = App.tg.initDataUnsafe?.start_param;
                    if (referrerId && !localStorage.getItem('referral_processed')) {
                        this.handleNewUserFromReferral(referrerId);
                    }
                },
                handleNewUserFromReferral(referrerId) {
                    console.log(`User was referred by: ${referrerId}`);
                    App.state.referrerId = referrerId;
                    App.state.bonusGenerations += 1;
                    App.stateManager.save();
                    localStorage.setItem('referral_processed', 'true');
                    App.ui.showPopup({
                        title: 'Бонус!',
                        message: 'Вы перешли по приглашению и получили 1 бонусную генерацию!',
                        buttons: [{ type: 'ok' }]
                    });
                },
                ...
                copyReferralLink() {
                    ...
                    try {
                        ...
                        App.tg.HapticFeedback.notificationOccurred('error');
                        App.ui.showPopup({ title: 'Ошибка', message: 'Не удалось скопировать ссылку.' });
                    }
                    document.body.removeChild(textarea);
                }
            },
            ...
            paymentManager: {
                initiateSubscription() {
                    App.ui.showPopup({ title: 'Подписка', message: 'Функция оплаты в разработке.' });
                    // Для тестирования можно вызвать активацию напрямую
                    this.activateSubscription();
                },
                activateSubscription(days = 30) {
                    const now = new Date();
                    App.state.subscription.isActive = true;
                    App.state.subscription.endDate = new Date(now.setDate(now.getDate() + days)).toISOString();
                    
                    // Логика начисления бонуса рефереру
                    if (App.state.referrerId) {
                        console.log(`ТРЕБУЕТСЯ БЭКЕНД: Начислить 10 генераций пользователю ${App.state.referrerId}`);
                        // Здесь должен быть вызов вашего API для начисления бонуса
                    }
            ...
            inspectorManager: {
                handleRenderDirectCode() {
                    const htmlContent = App.elements.directCodeInput.value;
                    if (!htmlContent.trim()) {
                        App.ui.showPopup({ title: 'Ошибка', message: 'Поле для кода пустое.' });
                        return;
                    }

                    App.elements.inspectorCodeOutput.textContent = htmlContent;
                    Prism.highlightElement(App.elements.inspectorCodeOutput);
                    
                    App.elements.inspectorPreviewIframe.srcdoc = htmlContent;
                    App.tg.HapticFeedback.notificationOccurred('success');
                    App.ui.toggleInspectorView('inspector-preview');
                },
                async handleLoadUrl() {
                    const url = App.elements.urlInput.value.trim();
                    if (!url) {
                        App.ui.showPopup({ title: 'Ошибка', message: 'Пожалуйста, введите URL.' });
                        return;
                    }

                    App.elements.inspectorLoader.style.display = 'flex';
                    App.elements.loadUrlButton.disabled = true;

                    try {
                        const htmlContent = await App.api.fetchUrlContent(url);
                        ...
                        App.ui.showPopup({ title: 'Ошибка загрузки', message: `Не удалось загрузить код с ${url}. Убедитесь, что URL правильный и сайт доступен. Ошибка: ${error.message}` });
                        App.tg.HapticFeedback.notificationOccurred('error');
                    } finally {
                        App.elements.inspectorLoader.style.display = 'none';
                        App.elements.loadUrlButton.disabled = false;
                    }
                }
            },
            ...
            generationManager: {
                async handleGeneration() {
                    ...
                    // Проверяем лимиты
                    const hasFreeGenerations = App.state.dailyGenerations.count < App.config.maxFreeGenerations;
                    const hasBonusGenerations = App.state.bonusGenerations > 0;

                    if (!App.state.subscription.isActive && !hasFreeGenerations && !hasBonusGenerations) {
                        App.ui.showPopup({
                    ...
                    if (!App.state.subscription.isActive) {
                        if (hasBonusGenerations) {
                            App.state.bonusGenerations--;
                        } else {
                    ...
                    try {
                        ...
                        App.stateManager.save();
                        App.ui.renderProfilePage(); // Обновляем UI профиля, чтобы показать списанную генерацию
                        App.tg.HapticFeedback.notificationOccurred('success');

                    } catch (error) {
                        console.error('Generation Error:', error);
                        indicator.remove();
                        const errorMessage = `Произошла ошибка: ${error.message}. Убедитесь, что вы указали правильный API-ключ.`;
            ...
            api: {
                ...
                async fetchUrlContent(url) {
                    ...
                    if (!response.ok) {
                        throw new Error(`Не удалось получить ответ от сервера. Статус: ${response.status}`);
                    }
